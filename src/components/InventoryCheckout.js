...
const InventoryCheckout = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get("https://hotel-sobana.herokuapp.com/inventory/")
      .then((res) => {
  ...
  useEffect(() => {
    //add a new property to each SELECTED array object (to store the selected quantity)
  ...
  const select = (e, data) => {
    //add new object to the SELECTED array
    e.preventDefault();
    if (checkSelect(data._id) != data._id) {
      //checking the new object id is already existing in the SELECTED array
      setselected((oldArray) => [...oldArray, data]);
    }
  };

  function checkSelect(id) {
    //checking all the IDs in the SELECTED array if matches the param(id)
  ...
  function deleteItem(e, data) {
    //deleting objects from the SELECTED array
    e.preventDefault();
    const newarry = selected.filter(function (item) {
  ...
  function addSelectedUnits(e, id, value) {
    ...
    if (value > 0) {
      const nextState = selected.map((a) =>
        a._id == id ? { ...a, selectedUnits: value } : a
      );
  ...
  useEffect(() => {
    ...
    for (let index = 0; index < selected.length; index++) {
      totalvalue +=
        selected[index].original_price * selected[index].selectedUnits;
      ...
      for (let x = 0; x < inventory.length; x++) {
        if (selected[index]._id == inventory[x]._id) {
          ...
          if (inventory[x].quantity >= selected[index].selectedUnits) {
            ...
            const total =
              parseInt(inventory[x].quantity) -
              parseInt(selected[index].selectedUnits);
            const newStockvalue = { total };
            //updating the new inventory stock quantity in the Database
            axios
              .put(
                `https://hotel-sobana.herokuapp.com/inventory/updatestock/${id}`,
                newStockvalue
              )
              .then(() => {})
              .catch((e) => {
            ...
            axios
              .post(" https://hotel-sobana.herokuapp.com/checkout/add", newItem)
              .then(() => {
                setselected([]);
                setto("");
                setdescription("");
  ...
  function checkoutPOP() {
    //funtion to display the CHECKOUT HISTORY TABLE
  ...
  return (
    <div className="display-box">
      <div className="header-box-sup">
        {" "}
        Checkout
        <button
          id="checkoutHistory-window-btn"
          onClick={checkoutPOP}
          style={{ display: "block" }}
        >
          History
      ...
      <div id="checkoutHistory-window" style={{ display: "none" }}>
        {" "}
        <History POP={checkoutPOP} />{" "}
      </div>
      ...
      <div className="checkout-box">
        <div className="checkoutlist cb">
          {selected.map(function (s) {
            return (
              <div className="checkoutlist-box">
                <li>
                  <span>{s.name}</span> - {s.model}{" "}
                </li>
        ...
        <div className="processcheckout cb">
          ...
          <div className="lower-cb">
            <span id="total-cb">
              Total : <span id="totalCheckoutPrice"></span>
            </span>
          ...
          <div className="display-all-inventory">
            <div className="itembox">
              {filtered
                ...
                .map(function (s) {
                  return (
                    <div className="round-checkout-box">

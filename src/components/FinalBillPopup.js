...
function Popup() {
  ...
  function saveFinalBill(e, mail) {
    ...
    if (result == true) {
      ...
      axios
        .post("https://hotel-sobana.herokuapp.com/FinalBill/AddBill", newBill)
        .then(() => {
          axios
            .put(`https://hotel-sobana.herokuapp.com/booking/PassCus/${id}`)
            .then(() => {
  ...
  useEffect(() => {
    axios
      .get(`https://hotel-sobana.herokuapp.com/booking/findOne/${id}`)
      .then((res) => {
  ...
  useEffect(() => {
    axios
      .get(`https://hotel-sobana.herokuapp.com/Meal_Order/Retrieve/${id}`)
      .then((res) => {
        if (res.data.length === 0) {
        } else {
  ...
  useEffect(() => {
    TotAmount(
      parseInt(BarOrder_Cost) +
        parseInt(Meal_Order_Cost) +
        parseInt(Allocationa_Amount) +
        parseInt(Additional_Bill)
    );
  });
      ...
      <dir className="blur-s">
        <div className="content-box-sfb">
          <div className="add-sup-head">
            ...
            <Link className="btn" to={"/front-office-manager/FinalBill"}>
              &times;
            </Link>
          </div>
          ...
          <div className="finalbill-content">
            <table setBarOrder="1">
              <tr>
                <th>Full Name </th> : <td> {CusName}</td>
              </tr>
              <tr>
                <th>NIC</th> : <td> {NIC}</td>{" "}
              </tr>
              <tr>
                <th>Allocation Cost</th> : <td> {Allocationa_Amount}</td>{" "}
              </tr>
              <tr>
                <th>Meal Order Cost</th> : <td> {Meal_Order_Cost}</td>{" "}
              </tr>
              <tr>
                <th>Bar Order Cost</th> : <td> {BarOrder_Cost}</td>{" "}
              </tr>
              <tr>
                <th>Additional Amount</th> : <td> {Additional_Bill}</td>{" "}
              </tr>
            </table>
            ...
            <fieldset hidden className="box" id="fs">
              <input
                type="text"
                class="searchTerm"
                placeholder="Enter Additional Bill Here"
                onChange={(e) => {
                  setAdditional(e.target.value);
                }}
                readonly
              />
              <button id="btn2" onClick={hideText}>
                Done
              </button>
            </fieldset>

            <button id="btn1" onClick={unHideText}>
            ...
            <button
              id="bt"
              onClick={(e) => saveFinalBill(e, meal.Mail_Address)}
            >
              Paid
            </button>
          </div>

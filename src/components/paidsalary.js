...
function ViewAll() {
  ...
  useEffect(() => {
    function getSalaries() {
      console.log("sucess");
      axios
        .get("https://hotel-sobana.herokuapp.com/salary/paidsalary")
        .then((res) => {
          setSalary(res.data);
        })
        .catch((err) => {
  ...
  return (
    <div className="display-box">
      <div className="header-box">
        <div>
          Paid Salary
          <button id="generate-reportt-btn" onClick={() => generatePDF(salary)}>
            Create Report
          </button>
        </div>
            ...
            {salary.map(function (salary) {
              return (
                <tr>
                  ...
                  <td>
                    <Link to={"/emp-manager/Viewonepayment/" + salary._id}>
                      {" "}
